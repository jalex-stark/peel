...
export async function buildReplay(events,{frequencyRanks=new Map(),dictionary=null,wordTierPolicy={}}={}){
 ...
 for(const [index,event] of distinct.entries()){
  ...
  const quality=wordQuality(event.state.board,frequencyRanks,wordTierPolicy);
  ...
  frames.push({...event,session,changed,removed,inspection:scoreInspection(event.state.board,score,{frequencyRanks,wordTierPolicy}),addedWords:previous?words.filter(w=>!previous.words.includes(w)):[],removedWords:previous?previous.words.filter(w=>!words.includes(w)):[],words,
   metrics:{quality:quality.score,overall:score.overall,hull:score.hullDensity,coverage:score.hullCoverage,commonness:score.commonness,solid:score.solidCoreScore,ponds:score.pondRank,longest:power.longest,strongest:power.maxScrabble},
 ...
 return {version:1,wordTierPolicy,createdAt:new Date().toISOString(),frames,sourceEvents:events.length,scoring:'All frames recalculated with the same current scoring rules and dictionary. This is retained history, which may start mid-session.'};
}
...
const names={quality:'Word quality',overall:'Overall',coverage:'Enclosed hull',hull:'Occupied hull',commonness:'Commonness',solid:'Solid core',ponds:'Ponds',longest:'Longest word',strongest:'Max Scrabble'};
...
function inspectionSVG(f,key){
 ...
 if(key==='quality')svg=(g.quality||[]).map(p=>'<rect class="heat quality-cell" data-tier="'+(p.tier||'none')+'" x="'+p.x*48+'" y="'+p.y*48+'" width="44" height="44" rx="5" fill="'+p.color+'"/>').join('');
...
if(!data.frames.every(f=>Number.isFinite(f.metrics.quality))){delete names.quality;$('metric').querySelector('[value=quality]').remove();}
$('metric').value=data.initialMetric||'overall';if(!$('metric').value)$('metric').value='overall';
