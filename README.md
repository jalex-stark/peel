...
**Words** opens a tier browser with **Allow all tiers**, **Ban F**, and **Ban D/F** presets and a minimum-tier selector. TWIXT/COIF/SUEDE are A, TURGID/VERACITY S, BY/BED/RUN/PARTY/DONOR B, QI C, AVO/SOU D, and PUL F. Search or browse any tier, and assign a personal rating or restore its default. Changes persist locally and immediately affect Auto-check, Peel, Free, and Suggest. The live score table shows tier badges. **Word quality** is a separate metric: each tile gets the lower tier of its across/down words (S=100, A=80, B=60, C=40, D=20, F=0), and the board value is their rounded mean. Tiles in no word are gray and score zero. Hover to color every board tile, or pin quality colors while editing—even with the score pane closed. The same grades appear in exported board artifacts and the replay metric selector, using current personal ratings consistently across all replay frames. Quality does not alter the base-score formula.

The initial editorial set is deliberately incomplete: curated A/S rewards expressive vocabulary; a provisional B fallback uses the top 20,000 entries in the bundled frequency ranking; other unreviewed words are provisional C, not automatically D/F. Only curated or personally rated low tiers are banned by the D/F presets. The initial cutoff allows all tiers to preserve existing games until the player chooses a filter. This is a game-preference taxonomy, not a claim about linguistic validity.

Individual additions and hides remain under **Individual word exceptions**. Added words still obey the tier cutoff, while explicit hides remain excluded. Free-board work orders have a persistent **Use challenge allowlist** toggle. Turning it off uses the normal dictionary and tier preferences while preserving the bank, geometry, anchor and edit-budget conditions. Connectivity and allowed words have independent checklist rows. Submissions record which word rules were used. Standalone warm-ups keep their fixed manifests. The versioned tier policy is recorded in QA/history events, local board-sharing snapshots and standalone board/metrics exports. Bare position codes continue to leave dictionary preferences local.
...
Word quality’s tier key shows word counts. Hover or keyboard-focus a tier to highlight all of its complete words (crossings included), with other tiles dimmed. The global heatmap still uses each tile’s weaker word. Coverage text distinguishes provisional ratings and default-C words.
