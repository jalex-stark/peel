...
export function cleanReplayEvents(events){
 ...
 return events.slice(-2000).flatMap((event,index)=>{
  try{
...
export function distinctReplayEvents(events){
 let previous=null;
 return cleanReplayEvents(events).filter(event=>{const key=replayStateKey(event.state);if(key===previous)return false;previous=key;return true;});
}
export async function buildReplay(events,{frequencyRanks=new Map(),dictionary=null}={}){
 ...
 for(const [index,event] of distinct.entries()){
  ...
  frames.push({...event,session,changed,removed,addedWords:previous?words.filter(w=>!previous.words.includes(w)):[],removedWords:previous?previous.words.filter(w=>!words.includes(w)):[],words,
