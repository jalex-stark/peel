...
let inputTour=null;
let localSharing=!navigator.webdriver,localShareTimer=null,localShareKey='',localShareState='Waiting to share',localClientId='';
try{localSharing=!navigator.webdriver&&localStorage.getItem('peel-local-sharing')!=='false';localClientId=localStorage.getItem('peel-client-id')||crypto.randomUUID();localStorage.setItem('peel-client-id',localClientId);}catch{localClientId=crypto.randomUUID();}
...
function scheduleLocalShare(force=false){
 ...
 const key=snapshot()+JSON.stringify(dictionaryOverrides)+JSON.stringify(scoreRun.modifierIds);if(key===localShareKey&&!force)return;
 clearTimeout(localShareTimer);
 localShareTimer=setTimeout(async()=>{
  try{const response=await fetch('/__peel/share',{method:'POST',headers:{'Content-Type':'application/json','X-Peel-Local':'1'},body:JSON.stringify({clientId:localClientId,state:JSON.parse(snapshot()),dictionaryOverrides,modifierIds:scoreRun.modifierIds})});
...
function qaRecord(action,details={}){
 ...
 qaJournal.events.push({sequence:(qaJournal.events.at(-1)?.sequence||0)+1,at:new Date().toISOString(),action,tool,details,selection:qaSelection(),state:{board:board.map(({id,l,x,y})=>({id,l,x,y})),rack:rack.map(({id,l})=>({id,l})),bunch:bag.length,view:{x:Math.round(pan.x),y:Math.round(pan.y),zoom}}});
...
Promise.all([fetch('/words.json').then(response=>{if(!response.ok)throw Error();return response.json();}),fetch('/common-words.json').then(response=>{if(!response.ok)throw Error();return response.json();})]).then(([words,frequent])=>{baseDictionary=new Set(words);dictionary=new Set(baseDictionary);for(const word of dictionaryOverrides.removed)dictionary.delete(word);for(const word of dictionaryOverrides.added)dictionary.add(word);commonWords=frequent;commonRanks=new Map(frequent.map((word,index)=>[word,index+1]));suggestionWords=frequent.slice(0,10000).filter(word=>word.length<=8).sort((a,b)=>b.length-a.length||commonRanks.get(a)-commonRanks.get(b));render();}).catch(()=>{status='Word data unavailable. Reload to enable checks and scoring.';render();});
