...
function qaRecord(action,details={}){
 ...
 qaJournal.events.push({eventId:crypto.randomUUID(),sequence:nextQaSequence(),at:new Date().toISOString(),action,tool,workOrderId:workOrder?.id||null,details,selection:qaSelection(),dictionaryOverrides:structuredClone(dictionaryOverrides),modifierIds:[...scoreRun.modifierIds],scoringVersion:'occupied-hull-max-length-v1',state:{board:board.map(({id,l,x,y})=>({id,l,x,y})),rack:rack.map(({id,l})=>({id,l})),bag:bag.map(({id,l})=>({id,l})),total,bunch:bag.length,view:{x:Math.round(pan.x),y:Math.round(pan.y),zoom}}});
...
function render(){
 ...
 $('dictionary-open').disabled=!baseDictionary||Boolean(workOrder);
 ...
 updateDemo();showWorkOrderMarks();showGhost(lastPointer);showSuggestion();showFinderMatches();showFreeTiles();showComponentBoundaries();if(scorePanelOpen)openScore(true);try{localStorage.setItem(gameStorageKey,snapshot());}catch{}scheduleLocalShare();
}
...
function openScore(refresh=false){
 ...
 const key=snapshot()+JSON.stringify(scoreRun)+JSON.stringify(dictionaryOverrides)+JSON.stringify(workOrderEntries);
...
function submitWorkOrder(){
 ...
 const result=assessPrototype(workOrder,{board,rack,bag,total});
 ...
 if(legal){workOrderEntries.push({at:new Date().toISOString(),value:result.value,changed:result.changed,passed:result.ok,state:JSON.parse(snapshot())});try{localStorage.setItem(gameStorageKey+'-submissions',JSON.stringify(workOrderEntries));}catch{toast('Submission storage is full; export this board to keep it.');}}
...
Promise.all([fetch(`${import.meta.env.BASE_URL}words.json`).then(response=>{if(!response.ok)throw Error();return response.json();}),fetch(`${import.meta.env.BASE_URL}common-words.json`).then(response=>{if(!response.ok)throw Error();return response.json();})]).then(([words,frequent])=>{baseDictionary=new Set(words);dictionary=new Set(baseDictionary);for(const word of dictionaryOverrides.removed)dictionary.delete(word);for(const word of dictionaryOverrides.added)dictionary.add(word);if(workOrder)dictionary=new Set(workOrder.words);commonWords=frequent;commonRanks=new Map(frequent.map((word,index)=>[word,index+1]));suggestionWords=frequent.slice(0,10000).filter(word=>word.length<=8).sort((a,b)=>b.length-a.length||commonRanks.get(a)-commonRanks.get(b));render();}).catch(()=>{status='Word data unavailable. Reload to enable checks and scoring.';render();});
