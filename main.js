...
import {TIERS,TIER_LABELS,normalizeTierPolicy,wordTier,tierAllowed,filteredDictionary} from './word-tiers.js';
import prototypes from './challenges/prototypes.json';
import {assessPrototype,prototypePanel,originalBoardInspection} from './challenges/prototype-rules.js';
...
const workOrder=prototypes.find(p=>p.id===new URLSearchParams(location.search).get('work-order'))||null;
const gameStorageKey=workOrder?`peel-work-order-${workOrder.id}-v${workOrder.version}`:'peel-game';
let workOrderUsesAllowlist=true;
try{workOrderUsesAllowlist=localStorage.getItem(gameStorageKey+'-allowlist')!=='false';}catch{}
let workOrderEntries=[];
try{workOrderEntries=JSON.parse(localStorage.getItem(gameStorageKey+'-submissions'))||[];}catch{}
let board=initialTiles(),rack=[],bag=PEELS.map((p,i)=>({id:`p${i}`,l:p.l})),total=33;
if(workOrder)({board,rack,bag,total}=structuredClone(workOrder.state));
...
app.innerHTML=`<header><div class="brand-wrap"><div class="brand"><div class="brand-icon"><i></i><i></i></div>peel<span>.</span></div><div class="header-note">A little word play.</div></div><div class="header-actions"><a class="text-btn challenge-link" href="${import.meta.env.BASE_URL}challenges/">Work orders</a><button class="text-btn qa-button" id="qa-open" title="Open the local QA journal"><span class="record-dot"></span> QA <span id="qa-count">0</span></button><button class="text-btn" id="help">${icon('help')} How to play</button><button class="new-btn" id="new">${icon('plus')} New game</button></div></header>
...
function scheduleLocalShare(force=false){
 ...
 const key=snapshot()+JSON.stringify(dictionaryOverrides)+JSON.stringify(wordTierPolicy)+workOrderUsesAllowlist+JSON.stringify(scoreRun.modifierIds)+(replayHistory.at(-1)?.at||'');if(key===localShareKey&&!force)return;
 clearTimeout(localShareTimer);
 localShareTimer=setTimeout(async()=>{
  try{const response=await fetch('/__peel/share',{method:'POST',headers:{'Content-Type':'application/json','X-Peel-Local':'1'},body:JSON.stringify({clientId:localClientId,state:JSON.parse(snapshot()),dictionaryOverrides,wordTierPolicy,workOrder:workOrder?{id:workOrder.id,useAllowlist:workOrderUsesAllowlist}:null,modifierIds:scoreRun.modifierIds,replayEvents:replayHistory})});
...
try{scorePanelOpen=Boolean(workOrder)||localStorage.getItem('peel-score-open')==='true';}catch{}
let scorePanelKey='';
let scoreInspection=null;
let densityRectangle=null;
let inspectedGeometry=null;
let inspectedWoven=null,inspectedPower=null,qualityPinned=false;
try{qualityPinned=localStorage.getItem('peel-quality-highlight')==='true';}catch{}
function showScoreInspection(){
 $('plane').querySelector('.score-inspection-layer')?.remove();
 $('plane').classList.remove('original-inspecting');
 ...
 for(const tile of $('plane').querySelectorAll('.quality-colored,.quality-muted')){tile.classList.remove('quality-colored','quality-muted');tile.style.removeProperty('--quality-color');delete tile.dataset.qualityTier;}
 if(scorePanelOpen&&scoreInspection?.startsWith('tier:')){
  const tier=scoreInspection.slice(5),quality=wordQuality(board,commonRanks||new Map(),wordTierPolicy);
  const words=quality.words.filter(w=>w.tier===tier),ids=new Set(words.flatMap(w=>w.ids));
  for(const cell of quality.cells){const tile=$('plane').querySelector(`[data-id="${cell.id}"]`);if(!tile)continue;
   const match=tier==='none'?cell.tier===null:ids.has(cell.id);tile.classList.toggle('quality-muted',!match);
   if(match){tile.classList.add('quality-colored','score-inspected');tile.style.setProperty('--quality-color',QUALITY_COLORS[tier]);tile.dataset.qualityTier=tier;}
  }
  $('inspection-caption').textContent=tier==='none'?`${quality.unworded} tiles belong to no word`:`${tier} tier · ${words.length} words: ${words.map(w=>w.word).join(', ')||'none'} · ${words.filter(w=>w.provisional).length} provisional`;
  $('inspection-caption').hidden=false;return;
 }
 if(qualityPinned||(scorePanelOpen&&scoreInspection==='quality')){
  const quality=wordQuality(board,commonRanks||new Map(),wordTierPolicy);
  for(const cell of quality.cells){const tile=$('plane').querySelector(`[data-id="${cell.id}"]`);if(tile){tile.classList.add('quality-colored');tile.style.setProperty('--quality-color',cell.color);tile.dataset.qualityTier=cell.tier||'none';}}
  $('inspection-caption').textContent='Word quality · weaker crossing wins · S 100 / A 80 / B 60 / C 40 / D 20 / F 0 · gray: no word';$('inspection-caption').hidden=false;
 }
 if(scoreInspection==='quality')return;
 if(!scorePanelOpen||!scoreInspection)return;
 if(scoreInspection==='original'&&workOrder){
  const cells=originalBoardInspection(workOrder,board),layer=document.createElement('div');layer.className='score-inspection-layer original-shadow-layer';$('plane').classList.add('original-inspecting');
  for(const cell of cells){const shadow=document.createElement('div');shadow.className=`original-shadow ${cell.changed?'changed':''} ${cell.occupied?'occupied':''}`;shadow.style.left=cell.x*48+'px';shadow.style.top=cell.y*48+'px';shadow.dataset.originalLetter=cell.l;shadow.textContent=cell.l;layer.append(shadow);}
  $('plane').append(layer);$('inspection-caption').textContent=`Starting board · ${cells.filter(c=>c.changed).length}/${workOrder.budget} positions changed. Orange shadows show the letters to restore; any copy of that letter counts.`;$('inspection-caption').hidden=false;return;
 }
...
function openBoardShare(){
 ...
 $('position-artifact').onclick=()=>{
  ...
  const html=createBoardArtifact(position,{frequencyRanks:commonRanks,modifierIds:scoreRun.modifierIds,dictionaryOverrides,wordTierPolicy});
...
const TIER_STORAGE='peel-word-tier-policy-v1';
let wordTierPolicy=normalizeTierPolicy();
try{wordTierPolicy=normalizeTierPolicy(JSON.parse(localStorage.getItem(TIER_STORAGE)));}catch{}
...
if(workOrder)scoreRun=emptyScoreRun();
function scoreRunSave(){if(workOrder)return;try{localStorage.setItem(SCORE_RUN_STORAGE,JSON.stringify(scoreRun));}catch{}}
...
function rebuildDictionary(){
 ...
 tierCatalog=[...new Set([...baseDictionary,...dictionaryOverrides.added])];
 dictionary=workOrder&&workOrderUsesAllowlist?new Set(workOrder.words):filteredDictionary(baseDictionary,commonRanks||new Map(),wordTierPolicy,dictionaryOverrides);
}
function wordTierUI(query=''){
 const counts=Object.fromEntries(TIERS.map(t=>[t,0]));let unreviewed=0;
 for(const word of tierCatalog){const r=wordTier(word,commonRanks||new Map(),wordTierPolicy);counts[r.tier]++;if(r.tier==='C'&&r.provisional)unreviewed++;}
 const blocked=[...new Set(getWords(board).map(w=>w.word))].filter(w=>!tierAllowed(wordTier(w,commonRanks,wordTierPolicy).tier,wordTierPolicy.minimum));
 return `<p>Choose how much word-game vocabulary you want. These are editorial preferences, not judgments about whether a word is real English.</p><div class="tier-presets">${[['F','Allow all tiers'],['D','Ban F'],['C','Ban D/F']].map(([minimum,label])=>`<button data-tier-minimum="${minimum}" aria-pressed="${wordTierPolicy.minimum===minimum}">${label}</button>`).join('')}</div><label class="tier-cutoff">Lowest allowed tier <select id="tier-minimum">${TIERS.map(t=>`<option value="${t}" ${wordTierPolicy.minimum===t?'selected':''}>${t} · ${TIER_LABELS[t]}</option>`).join('')}</select></label><p id="tier-effect" role="status">${dictionary.size.toLocaleString()} words allowed. ${blocked.length?`Blocked on this board by tiers: ${blocked.join(', ')}.`:'No words on this board are blocked by tiers.'} Auto-check, Peel, Free and Suggest use this cutoff.</p><div class="tier-legend">${TIERS.map(t=>`<div><b class="tier-letter tier-${t}">${t}</b><span>${TIER_LABELS[t]}<small>${counts[t].toLocaleString()} words</small></span></div>`).join('')}</div><p class="tier-explanation">Ratings combine editorial anchors and a Haiku batch pass. Generated ratings are labeled and editable. B includes everyday words; frequency-based B ratings are provisional. ${unreviewed.toLocaleString()} unreviewed words are provisionally C, so unfamiliarity alone never makes a word D/F. You can change any rating below.</p><div class="tier-search"><input id="tier-query" value="${dictionaryWord(query)}" placeholder="Find a word, e.g. veracity" aria-label="Find word tier" autocomplete="off" spellcheck="false"><select id="tier-filter" aria-label="Browse tier"><option value="">All tiers</option>${TIERS.map(t=>`<option>${t}</option>`).join('')}</select></div><div id="tier-results" class="tier-results"></div><p class="tier-explanation">Personal tier ratings and the cutoff save in this browser. Tier choices do not change the board's scoring formula.</p>`;
}
function applyTierPolicy(next,action,details,query=''){
 wordTierPolicy=normalizeTierPolicy(next);try{localStorage.setItem(TIER_STORAGE,JSON.stringify(wordTierPolicy));}catch{toast('Could not save tier preferences.');}
 rebuildDictionary();suggestion=null;render();qaRecord(action,details);openDictionary('',query);
}
function bindWordTierUI(query=''){
 const input=$('tier-query'),filter=$('tier-filter');let tierLimit=40;
 const update=(reset=true)=>{
  if(reset)tierLimit=40;
  const value=dictionaryWord(input.value);input.value=value;
  let words=value?tierCatalog.filter(w=>w.includes(value)):filter.value?tierCatalog:[...new Set([...getWords(board).map(w=>w.word),...'TWIXT TURGID COIF SUEDE VERACITY BY BED RUN PARTY DONOR QI AVO PUL SOU'.split(' '),...Object.keys(wordTierPolicy.overrides)])];
  if(filter.value)words=words.filter(w=>wordTier(w,commonRanks,wordTierPolicy).tier===filter.value);
  if(value)words.sort((a,b)=>(a===value?-1:b===value?1:0)||(Number(b.startsWith(value))-Number(a.startsWith(value)))||a.length-b.length||a.localeCompare(b));
  const total=words.length;words=words.slice(0,tierLimit);
  $('tier-results').innerHTML=words.map(w=>{const r=wordTier(w,commonRanks,wordTierPolicy),allowed=dictionary.has(w);return `<div class="tier-word"><b class="tier-letter tier-${r.tier}">${r.tier}</b><span><strong>${w}</strong><small>${r.source} · ${allowed?'allowed':'blocked / not in dictionary'}</small></span><select data-word-tier="${w}" aria-label="Tier for ${w}"><option value="auto">Default (${wordTier(w,commonRanks).tier})</option>${TIERS.map(t=>`<option value="${t}" ${wordTierPolicy.overrides[w]===t?'selected':''}>${t}</option>`).join('')}</select></div>`;}).join('')+`<p>${total?`Showing ${words.length} of ${total.toLocaleString()} matches.`:'No matching words. Missing words can be added under individual exceptions.'}</p>${total>words.length?'<button id="tier-more">Show more words</button>':''}`;
  if($('tier-more'))$('tier-more').onclick=()=>{tierLimit+=40;update(false);};
  for(const select of document.querySelectorAll('[data-word-tier]'))select.onchange=()=>{
   const word=select.dataset.wordTier,overrides={...wordTierPolicy.overrides};if(select.value==='auto')delete overrides[word];else overrides[word]=select.value;
   applyTierPolicy({...wordTierPolicy,overrides},'dictionary.tier',{word,tier:select.value},input.value||word);
  };
 };
 input.oninput=update;filter.onchange=update;update();
 const changeMinimum=minimum=>applyTierPolicy({...wordTierPolicy,minimum},'dictionary.cutoff',{minimum},input.value);
 $('tier-minimum').onchange=e=>changeMinimum(e.target.value);
 for(const button of document.querySelectorAll('[data-tier-minimum]'))button.onclick=()=>changeMinimum(button.dataset.tierMinimum);
}
...
function dictionaryChange(word,action){
 ...
 dictionaryOverrides={added:[...added].sort(),removed:[...removed].sort()};dictionarySave();rebuildDictionary();
 suggestion=null;render();qaRecord(`dictionary.${action}`,{word,state:dictionaryWordState(word)});openDictionary(word);
}
function openDictionary(initial='',tierQuery=''){
 const word=dictionaryWord(initial),state=word?dictionaryWordState(word):'';
 modal(`<h2>Word tiers.</h2>${wordTierUI(tierQuery)}<details id="dictionary-advanced" ${initial?'open':''}><summary>Individual word exceptions</summary><p>Add a missing word or hide one explicitly. Added words still follow your tier cutoff; hidden words stay hidden.</p><div class="dictionary-entry"><input id="dictionary-word" value="${word}" maxlength="24" autocomplete="off" spellcheck="false" placeholder="Type a word" aria-label="Dictionary word"><button id="dictionary-action" ${word.length<2?'disabled':''}>${state==='bundled'||state==='custom'?'Hide word':state==='hidden'?'Restore word':'Add word'}</button></div><div class="dictionary-state" id="dictionary-state">${word?`${word} · ${state==='custom'?'your addition':state==='hidden'?'hidden from bundled list':state==='bundled'?'bundled word':'not included'}`:'Letters A–Z, at least two characters.'}</div><div class="dictionary-lists"><section><h3>Your additions</h3>${dictionaryOverrides.added.length?dictionaryOverrides.added.map(item=>`<button data-dictionary-remove="${item}">${item} ${icon('x')}</button>`).join(''):'<p>None yet.</p>'}</section><section><h3>Hidden bundled words</h3>${dictionaryOverrides.removed.length?dictionaryOverrides.removed.map(item=>`<button data-dictionary-restore="${item}">${item} ${icon('return')}</button>`).join(''):'<p>None yet.</p>'}</section></div></details>`);
 ...
 $('tier-query').closest('.modal').classList.add('word-tier-modal');bindWordTierUI(tierQuery);if(initial){input.focus();input.setSelectionRange(input.value.length,input.value.length);}else $('tier-query').focus({preventScroll:true});
}
...
async function openReplay(events=null){
 ...
 try{
  ...
  const result=await buildReplay(events,{frequencyRanks:commonRanks,dictionary,wordTierPolicy});
...
function qaRecord(action,details={}){
 ...
 qaJournal.events.push({eventId:crypto.randomUUID(),sequence:nextQaSequence(),at:new Date().toISOString(),action,tool,workOrderId:workOrder?.id||null,workOrderWordRules:workOrder?(workOrderUsesAllowlist?'allowlist':'personal'):null,details,selection:qaSelection(),dictionaryOverrides:structuredClone(dictionaryOverrides),wordTierPolicy:structuredClone(wordTierPolicy),modifierIds:[...scoreRun.modifierIds],scoringVersion:'occupied-hull-max-length-v1',state:{board:board.map(({id,l,x,y})=>({id,l,x,y})),rack:rack.map(({id,l})=>({id,l})),bag:bag.map(({id,l})=>({id,l})),total,bunch:bag.length,view:{x:Math.round(pan.x),y:Math.round(pan.y),zoom}}});
...
function render(){
 ...
 if(!(workOrder&&workOrderUsesAllowlist)&&autoCheck&&dictionary){const blocked=[...new Set(getWords(board).filter(w=>badIds.has(w.ids[0])&&!tierAllowed(wordTier(w.word,commonRanks||new Map(),wordTierPolicy).tier,wordTierPolicy.minimum)).map(w=>`${w.word} (${wordTier(w.word,commonRanks||new Map(),wordTierPolicy).tier})`))];if(blocked.length)status=`Tier cutoff blocks ${blocked.join(', ')}. Change tiers in Words.`;}
 ...
 $('dictionary-open').disabled=!baseDictionary||Boolean(workOrder&&workOrderUsesAllowlist);
 ...
 updateDemo();showWorkOrderMarks();showGhost(lastPointer);showSuggestion();showFinderMatches();showFreeTiles();showComponentBoundaries();if(scorePanelOpen)openScore(true);else showScoreInspection();try{localStorage.setItem(gameStorageKey,snapshot());}catch{}scheduleLocalShare();
}
...
function openScore(refresh=false){
 ...
 const key=snapshot()+JSON.stringify(scoreRun)+JSON.stringify(dictionaryOverrides)+JSON.stringify(wordTierPolicy)+JSON.stringify(workOrderEntries)+qualityPinned+workOrderUsesAllowlist;
 ...
 const power=wordPower(board),quality=wordQuality(board,commonRanks,wordTierPolicy);
...
function submitWorkOrder(){
 pause();if(keyboardPlacement){toast('Place your floating tiles first.');return;}
 const result=assessPrototype(workOrder,{board,rack,bag,total},{dictionary,useAllowlist:workOrderUsesAllowlist});
 // Save legal below-target experiments too, so the player can compare tradeoffs.
 const legal=result.checks.filter(c=>!c.label.includes('/ target')).every(c=>c.ok);
 if(legal){workOrderEntries.push({at:new Date().toISOString(),value:result.value,changed:result.changed,passed:result.ok,wordRules:workOrderUsesAllowlist?'allowlist':'personal',wordTierPolicy:structuredClone(wordTierPolicy),state:JSON.parse(snapshot())});try{localStorage.setItem(gameStorageKey+'-submissions',JSON.stringify(workOrderEntries));}catch{toast('Submission storage is full; export this board to keep it.');}}
 openScore();$('work-order-notice').textContent=result.ok?'Accepted. Keep experimenting or save another arrangement.':`Still needed: ${result.reason}`;
 qaRecord('work_order.submit',{level:workOrder.id,accepted:result.ok,legal,value:result.value,changed:result.changed});
}
function showWorkOrderMarks(){
 if(!workOrder)return;
 const b=workOrder.bounds;
 $('plane').insertAdjacentHTML('beforeend',`<div class="work-order-boundary" style="left:${b.minX*48-2}px;top:${b.minY*48-2}px;width:${(b.maxX-b.minX+1)*48}px;height:${(b.maxY-b.minY+1)*48}px"></div>`);
 for(const tile of board)if(workOrder.locks.some(t=>t.x===tile.x&&t.y===tile.y&&t.l===tile.l))$('plane').querySelector(`[data-id="${tile.id}"]`)?.classList.add('work-order-anchor');
 $('peel').innerHTML='Submit work order';
}
...
try{const stored=JSON.parse(localStorage.getItem(gameStorageKey));if(stored&&Array.isArray(stored.board)&&Array.isArray(stored.rack)&&Array.isArray(stored.bag)){({board,rack,bag,total}=stored);status='Welcome back. Your words are right where you left them.';}}catch{}
if(workOrder){document.body.classList.add('work-order-mode');document.querySelector('.intro h1').textContent=workOrder.title;document.querySelector('.intro p').textContent='Free-board work order · progress saves separately from your main board.';document.querySelector('.intro .eyebrow').textContent='PROTOTYPE · EXPERIMENT, REARRANGE, COMPARE';}
fit();new ResizeObserver(()=>{if(!drag)fit();}).observe($('board'));
if(workOrder)qaRecord('position.import',{source:'work_order',level:workOrder.id,version:workOrder.version});
else if(qaJournal.active)retainReplay({sequence:qaJournal.events.at(-1)?.sequence||0,at:new Date().toISOString(),action:'session.resume',state:{board,rack,bunch:bag.length}});
Promise.all([fetch(`${import.meta.env.BASE_URL}words.json`).then(response=>{if(!response.ok)throw Error();return response.json();}),fetch(`${import.meta.env.BASE_URL}common-words.json`).then(response=>{if(!response.ok)throw Error();return response.json();})]).then(([words,frequent])=>{baseDictionary=new Set(words);dictionary=new Set(baseDictionary);for(const word of dictionaryOverrides.removed)dictionary.delete(word);for(const word of dictionaryOverrides.added)dictionary.add(word);commonWords=frequent;commonRanks=new Map(frequent.map((word,index)=>[word,index+1]));rebuildDictionary();suggestionWords=frequent.slice(0,10000).filter(word=>word.length<=8).sort((a,b)=>b.length-a.length||commonRanks.get(a)-commonRanks.get(b));render();}).catch(()=>{status='Word data unavailable. Reload to enable checks and scoring.';render();});
