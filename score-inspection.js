...
export function scoreInspection(tiles,score){
 ...
 return {hull:hullInspection(tiles,geometry.hull),solid:geometry.solid,ponds:inspectionCellPath(score.pondRegions.flatMap(r=>r.cells)),longest:inspectionCellPath(wordTiles(power.longestWords)),strongest:inspectionCellPath(wordTiles(power.maxScrabbleWords)),commonness:tiles.map(t=>{const words=score.words.filter(w=>w.ids.includes(t.id));return {x:t.x,y:t.y,value:words.length?words.reduce((sum,w)=>sum+w.commonness,0)/words.length:0};})};
}
